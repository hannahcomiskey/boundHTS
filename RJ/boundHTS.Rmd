...
To begin, for time $t$ we will denote the top series as $u_{t}$ and the bottom series $m$ as $b_{m, t}$. We assume the bottom series is given by $b_{m, t} \sim \mathrm{Poisson}(\lambda_{m,t})$ and let the mean of these forecasts be given by $\hat{\lambda}_{m,t}$. Taking advantage of the summation property of Poisson distributed variables, the expected value of the convoluted density is given by $\sum_{i=1}^{j}\lambda_{i,t}$. Via convolution, the estimated PMF of the top series node is given by,
\begin{align}
    f_{m+1,t}(u_{t}) &=
