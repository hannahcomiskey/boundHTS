...
poisson_sim_data$Tot_edit <- pmax(0, poisson_sim_data$Tot + sample(c(-10:10), length(poisson_sim_data$Tot), replace=TRUE))
...
glm_results <- rolling_poisson_glm(sim_data = poisson_sim_data,
                                   ...
                                   series_names = c("Tot_edit", "AA", "AB", "BA", "BB"))
...
top_y_vals <- seq(from = 0, to = max(poisson_sim_data$Tot_edit)+10)
...
means_df <- data.frame(
  node = names(lambda_t),
  mean = lambda_t
  ) %>% mutate(node = ifelse(node=="Tot_edit", "Total", node))
