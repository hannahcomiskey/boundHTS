...
Each convolution function takes the following parameters:

- `z_values`: A fine grid over which the resulting aggregated density is calculated. This may be integer-valued or defined over the unit interval.  
- `XXX_input`: A set of distribution-specific parameters, provided either as matrices of posterior samples or vectors of point estimates. The name of these inputs will vary with the class of distribution.
- `point`: A logical (`TRUE`/`FALSE`) indicating whether inputs are point estimates (`point = TRUE`) or posterior samples (`point = FALSE`).

In addition, the package includes the `tilt_density()` function, which applies exponential tilting to adjust a predictive density toward a specified target mean. This function works for both discrete and continuous distributions, offering a flexible post-processing step for reconciling or calibrating forecasts. The `tilt_density()` function takes takes the following parameters:

- `mu_theory`: The predictive mean which the convoluted density is tilted towards.
- `z_values`: A fine grid over which the resulting aggregated density is calculated over, this may be integer valued or over the unit interval. This must match the grid used for the convolution step.
- `f_y`: The estimated convoluted density that has been calculated over the interval. 
- `discrete`: A binary `TRUE`/`FALSE` statement to indicate continuous probability density functions (`discrete=FALSE`) or discrete probability mass functions (`discrete=TRUE`) being supplied.

The workflow of the proposed methodology is summarised in Figure~\ref{fig:function_flow}, where you first determine the classification of data being used (discrete or continuous) and then the type of parameter estimates being supplied (point estimates or posterior samples).

```{r, echo=FALSE, fig.cap="A flow diagram of the functions to be used in the `boundhts` package.\\label{fig:function_flow}", out.width="100%"}
knitr::include_graphics("figures/functions_flowdiagram.pdf")
```
...
```{r, echo=FALSE, fig.cap="The two-level hierarchical set up of the Poisson simulation. The four child nodes (A, B, C, D) aggregate to the parent node, Total.\\label{fig:sim_poisson_setup}", out.width="100%"}
...
The next step is to generate the aggregated series density using convolution. This is accomplished with the `Poisson_convolution()` function, which takes the grid of discrete values (`z_values`), estimates of the bottom-level series (`lambda_vector`) and sets `point=TRUE` for using point estimates. Once the convolution density is computed, it is adjusted, or "tilted", to align with the predictive mean of the top-level series. The `tilt_density()` function performs this adjustment, taking as inputs the predictive mean, the grid of discrete values, and the convoluted density.
...
ggplot(df_plot, aes(x = y, y = pmf, fill = Density)) +
  ...
  geom_vline(aes(xintercept = poisson_sim_data$Tot[t_idx], linetype = "Observation"), color = "black", lwd = 1.2) +
...
```{r, echo=FALSE, fig.cap="The two-level hierarchical set up of the Beta simulation. The two child nodes (AA, AB) aggregate to the parent node, A. \\label{fig:sim_beta_setup}", out.width="100%"}
...
```{r, fig.cap="The tilted and convoluted density of the top series using Beta distributed bottom series nodes for a given time point. The dark red colour captures the density of the convolution, the blue represents the tilted denisty and the dashed black line indicates the predictive mean of the top series. \\label{fig:beta-tilted-plot}"}
df_plot <- data.frame(
  y = rep(z_values,2),
