...
alpha_mat <- beta_params_df %>% select(Node, Alpha) %>% pivot_wider(names_from = Node, values_from = Alpha)

beta_mat <- beta_params_df %>% select(Node, Beta) %>% pivot_wider(names_from = Node, values_from = Beta)

# Set up weights
weights_list <- list(A = c(0.75, 0.25))
