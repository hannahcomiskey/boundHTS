...
n_testyears <- c(max(sim_data$Time)-5):max(sim_data$Time)
...
mu_top_post <- dplyr::as_tibble(mu_post[[1]]) %>%
  ...
  tidyr::pivot_longer(cols = -sample_id, names_to = "param", 
                      values_to = "estimate") %>%
  dplyr::mutate(param = gsub("mu\\[|\\]", "", param)) %>%
  tidyr::separate_wider_delim( param, names = c("Time", "index_top"),
                               delim = ",") %>%
  dplyr::mutate(Time = as.numeric(gsub("mu\\[", "", Time)),
                index_top = gsub("\\]", "", index_top),
                Node = ifelse(index_top == 1, "A", NA)) %>%
...
mu_bottom_post <- dplyr::as_tibble(mu_post[[2]]) %>%
  ...
  tidyr::pivot_longer(cols = -sample_id, names_to = "param", 
                      values_to = "estimate") %>%
  dplyr::mutate(param = gsub("mu\\[|\\]", "", param)) %>%
  tidyr::separate_wider_delim( param, names = c("Time", "index_bottom"),
                               delim = ",") %>%
  dplyr::mutate(Time = as.numeric(gsub("mu\\[", "", Time)),
                index_bottom = gsub("\\]", "", index_bottom),
                Node = ifelse(index_bottom == 1, "AA", "AB")) %>%
...
pred_ysim <- preds_mean_all %>%
  dplyr::as_tibble() %>%
  dplyr::rename(AA = pred_AA, AB = pred_AB) %>%
...
test_years <- sim_data %>% 
  dplyr::select(Time) %>% 
  dplyr::filter(Time >= 240) %>% # validation test set
  unlist() %>% 
  as.vector()
...
for (i in seq_along(test_years)) {
  ...
  P_top <- post_ysim %>% dplyr::filter(Time==t_i) %>% 
    dplyr::ungroup() %>% 
    dplyr::select(dplyr::all_of(top_node))
  P_bottom <- post_ysim %>% dplyr::filter(Time==t_i) %>% 
    dplyr::ungroup() %>% 
    dplyr::select(dplyr::all_of(bottom_nodes))

  # Extract mean phi for top + bottom
  phi_top <- as.numeric(pred_ysim[i, "phi_A"])
