...
```{r}
hts_data_long <- tidyr::pivot_longer(poisson_sim_data, cols = -c(X), names_to = "Level", values_to = "Value")
...
for (t in seq_along(test_indices)) {
  ...
  for (series in colnames(ally)) {
    ...
    fit <- stats::glm(
      ...
      data    = poisson_sim_data[train_idx, c("X", series)],
      family = "poisson"
    )
    ...
    lambda_est[test_indices[t], series] <-
      stats::predict(fit, newdata = poisson_sim_data[test_indices[t], c("X", series)])
...
for(t in 1:length(test_indices)) {
  ...
  f_y_top <- Poisson_convolution_density_point_parallel(z_values = top_y_vals,
                                                        lambda_point = as.numeric(lambda_bseries))
  
  # Tilt density
  tilted_dens <- tilt_density(mu_theory[k], top_y_vals, f_y, discrete=TRUE) # tilt density
