...
for(t in seq_along(n_testyears)) {
  ...
  if(t==1) {
    pred_A <- tibble(mean_prop = predict(zib_AA, type = "response")) %>% 
      mutate(Node = "A", Time = 2:length(train_idx))
    pred_AA <- tibble(mean_prop =  predict(zib_AA, type = "response")) %>% 
      mutate(Node = "AA", Time = 2:length(train_idx))
    pred_AB <- tibble(mean_prop =  predict(zib_AA, type = "response")) %>% 
  ...
  mu_pred_AA <- predict(zib_AA, newdata = sim_data[n_testyears[t], ],
                        what = "mu", type = "response")
  mu_pred_AB <- predict(zib_AA, newdata = sim_data[n_testyears[t], ],
                        what = "mu", type = "response")
  
  # Get variance of one-step ahead forecast
  var_beta_A  <- predict(zib_A, newdata = sim_data[n_testyears[t], ],
                        what = "sigma", type = "response")^2
  var_beta_AA  <- predict(zib_AA, newdata = sim_data[n_testyears[t], ],
                        what = "sigma", type = "response")^2
  var_beta_AB  <- predict(zib_AB, newdata = sim_data[n_testyears[t], ],
                        what = "sigma", type = "response")^2
